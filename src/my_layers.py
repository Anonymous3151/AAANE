...
class WeightedAspectEmb(Layer):
    ...
    def build(self, input_shape):
        ...
        self.W = self.add_weight((self.input_dim, self.output_dim), initializer=self.init)
...
class Square_loss(Layer):
    def __init__(self,  **kwargs):
        super(Square_loss, self).__init__(**kwargs)

    def call(self, input_tensor, mask=None):
        predict  = input_tensor[0]
        lable_ls = input_tensor[1]
        # print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
        #
        # print(predict.ndim, type(predict))
        # print(lable_ls.ndim, type(lable_ls))

       # print(predict.shape.eval())

        # loss = K.dot(K.update_sub(predict, lable_ls), K.update_sub(predict, lable_ls))
        loss = K.dot(predict-lable_ls, K.transpose(predict-lable_ls))

        #loss = K.abs(predict - lable_ls)
        return loss
